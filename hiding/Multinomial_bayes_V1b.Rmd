...
##Summary
This is an alternative version of the basic NB workflow for classifying TA comments. The code is slightly different than what was used for Categorical NB testing, but takes in text using the same command set as NB-Ver 2.0. This alternative method lets me perform true multinomial analysis. I can switch between using binary yes/no categorization of words' presence or absence, and the actual frequencies. There is a trade-off though; when using non-categorical values, I am limited to two bins: in-group versus out-group. If I use multiple groups, the data fail to parse correctly. If I want to look at multiple bins for classification, I have to use categorical values.

Overall categorical NB seems to perform better than true multinomial NB. I get 75-84% overall accuracy (in vs out) if I use yes/no categories for both Subject and Structure, but accuracy drops to ~65% when I use the raw numeric frequencies. Reducing the Laplace value does not affect overall performance.
