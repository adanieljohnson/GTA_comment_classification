...
##Summary
This it the current Naive Bayes workflow used to classify TA comments based on Subject and Structure. Additions and revisions made to **Version 2r.2c** are:

*  Random selection of data rows used for training and test datasets. 
*  Final output matrix tables are more informative. 
*  Addition of a back-analysis step that reviews entire dataset, not just testing sub-set.
*  Outputs a tidy data table in CSV format that contains the original comments (pre-clean-up) plus their hand-scored and predicted subject and structure. 
...
```{}
# Alternate version of convert_function code block. This version KEEPs word frequencies while still using the same variable names and code structure. The resulting matrices have cells with word frequencies instead. 
#Only works when the NB classifier is looking at ingroup/outgroup. Breaks on multiple categories.
#
convert_counts2 <- function(x) {
  y <- x
}
...
```{r bayes_training}
# Train the classifier on training data, then test on test subset and full dataset.
...
#The truth table lists how many of the predicted categories are in their correct categories. Adding diagonally and dividing by total count gives the percent accuracy at predicting subject. this is what the confusion matrix calculates. 

#The faster way is to calculate a confusion matrix, which includes BOTH the truth table, and the final overall accuracy.

#Alternatively, the Cross Table command below (from "gmodels" package) generates a more detailed output table with frequency values for each individual cell.
...
##Graph Outputs
Comparing outcome from Hand-coding vs. Bayes

Need to combine two datasets into one. 

```{r}
ggplot(data=proptallysum1, aes(`Hand.Coded.Subject`, `total`,group=1))+
  geom_line()
