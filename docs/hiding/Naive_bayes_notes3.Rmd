...
**Original Version**
Performs tokenization using the "DocumentTermMatrix" function. This creates a matrix in which the rows indicate documents (SMS messages in this case) and the columns indicate words. It should be noted that the "DocumentTermMaxtrix" function has the power to do all of the text mining above in one command.
```{r}
...
**Quick code block to generate 2-grams and 3-grams from full corpus of TA comments**
...
text.corpus = sms_corpus_clean2

**This code block used tm's NLP generate n-grams from full corpus of TA comments**
...
This should build a doc-terms matrix tokenize using n-grams 
```{r}
library(topicmodels)
library(RWeka)
corpus <- sms_corpus_clean2 
